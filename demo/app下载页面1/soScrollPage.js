...
(function ($) {
	...
	function F(opt) {
		...
		this.betweenCls = opt.betweenCls;
	...
	F.prototype ={
		...
		setH : function () {
			...
			if(t.minHeight){
				wh = wh > t.minHeight ? wh : t.minHeight;
			}
			t.wh = wh;
			t.$box.height(wh);
			t.$page.height(wh);
		...
		setBetween : function() {
			...
			t.$page.each(function (i,v) {
				if(t.betweenCls){
					var bh = $(this).find(t.betweenCls).innerHeight();
					var wh = $(window).height();
					if(t.minHeight){
						wh = wh > t.minHeight ? wh : t.minHeight;
					}
					$(this).find(t.betweenCls).css({"top":(wh-bh)/2 + "px"});
				}
			});
		},
		//添加固定class
		...
		setThumb : function (i) {
			var t = this;
			t.$thumb.removeClass('current').eq(i).addClass('current');
		},
		//缩略图事件
		clickThumb : function () {
			var t = this;
		...
		animatePage : function (ix) {
			...
			t.cellsAnimate(t.prev,'out',function () {//先执行prev退出事件
				t.setThumb(t.now);
				t.$scrollbox.stop().animate({top:-t.wh*t.now+'px'},800,'easeInOutExpo',function () {//页面切换事件
	...
	$.fn.extend({
		soScrollPage : function (o) {
			var o = $.extend({
				box : $(this),//包裹对象为自身
				scrollbox : '#scrollbox',//滚动包裹对象
				page : '.page',//页
				pageClsH : 'scrollpage-',//为每页单独定义的cls头
				thumbCls : 'pageThumb',//thumb class
				betweenCls: false,//垂直居中内容class
				minDuring : 300,//每页离开时最少停留时间
				minHeight: false,//是否设定页面最小高度
				animateOpt : null,//动画参数对象
				callback : function(ix){} //切完页面完成后的回调函数
			},o||{});
